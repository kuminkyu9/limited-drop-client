...
interface itemProps {
  entrieList: EntrieProps[];
}

const EntriesTabMain = ({ entrieList }: itemProps) => {
  return (
    <div className="space-y-4">
      {entrieList.length == 0 ? <div className="flex h-52 items-center justify-center rounded border border-dashed border-gray-200 text-base text-gray-400">
        아직 응모 내역이 없습니다.
      </div> : entrieList.map((item) => (
